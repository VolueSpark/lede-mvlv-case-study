...
def meter():
    ...
    k = int(request.args.get('k', default=0))
    ...
    n = len(data.select(r'^X_.*$').columns)
    fig = make_subplots(
        rows=n, cols=1,
        subplot_titles= data.select(r'^X_.*$').columns,
    )

    for i, meter_id in enumerate(data.select(r'^X_.*$').columns):
        target =data.filter((pl.col('type')=='target') & (pl.col('offset')==k+data['offset'].min()))
        forecast = data.filter((pl.col('type')=='forecast') & (pl.col('offset')==k+data['offset'].min()))
