...
def plot_active_power(x: pl.DataFrame, y: pl.DataFrame, y_hat: pl.DataFrame):
    x = x.with_columns(p_sum_kwh=x.select(pl.col(r'^X_P.*$')).sum_horizontal())
    y = y.with_columns(p_sum_kwh=y.select(pl.col(r'^X_P.*$')).sum_horizontal())
    y_hat = y_hat.with_columns(p_sum_kwh=y_hat.select(pl.col(r'^X_P.*$')).sum_horizontal())
